...
const Donate = () => {
  ...
  useEffect(() => {
    const fetchCampaigns = async () => {
      ...
      try {
        ...
        const response = await fetch(`https://dana-mitra-zehy.vercel.app/api/donate/${encodeURIComponent(referralCode)}`);
  ...
  const handleDonate = async (e) => {
    ...
    try {
      ...
      const response = await fetch("https://dana-mitra-zehy.vercel.app/api/donate", {
        method: "POST",
      ...
      const options = {
        ...
        handler: async (response) => {
          const verifyResponse = await fetch("https://dana-mitra-zehy.vercel.app/api/donate/verify", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
      ...
      <Box sx={{ bgcolor: "background.default", minHeight: "100vh" }}>
        ...
        <Box sx={{ p: 4, bgcolor: "white", boxShadow: "0px 6px 20px rgba(0,0,0,0.1)", mb: 6, borderRadius: 3, border: `2px solid ${theme.palette.primary.main}` }}>
          <Container maxWidth="md">
            ...
            >
              DanaMitra is a platform dedicated to empowering communities through impactful campaigns. As part of the NayePankh Foundation, a distinguished NGO based in Noida, Uttar Pradesh, we are recognized by the government with 80G & 12A registration and featured in esteemed publications like The Pioneer, Dainik Jagran, and Hindustan. We engage with over 475 children across 10 shelter homes, 3 community centers, and 5 villages, fostering holistic growth and development. Join us in creating a brighter future for underprivileged children by supporting our campaigns!
            </Typography>
          </Container>
        </Box>
